...
agents/                       Agent persona definitions (8 agents)
...
4. **Phase 4**: Code Reviewer + Devil's Advocate per implementation stream, scoped to the files listed in that stream's manifest (parallel batches of 6). Code review always runs — prefers the global `code-reviewer` agent if installed, otherwise uses the bundled `fleet-code-reviewer`
...
- Phase 4 code review always runs — uses the global `code-reviewer` agent (`~/.claude/agents/code-reviewer.md`) if installed, otherwise falls back to the bundled `fleet-code-reviewer` agent
